...
import { CommunityLists, communitylisting, communitylistingfilter, FILE, comments  } from '../../models/firebase-community.model';
...
@Component({
  ...
  templateUrl: './categorieslist.page.html',
  styleUrls: ['./categorieslist.page.scss'],
})
export class CategorieslistPage implements OnInit {
  ...
  showdata_array: Array<communitylisting> = [];
  showdata: Array<communitylisting> = [];
  ...
  constructor(
    ...
    public communitylistservice: CommunitylistserviceService,
  ...
  async ionViewDidEnter()
  {
    ...
    if (localStorage.getItem('player_id'))
    {
      ...
      {
        this.userservice.update_player_id(localStorage.getItem('user'), localStorage.getItem('player_id'))
      }
    }
    ...
    this.posservice.getpos().then(pos => 
      {
        this.storage.get('Latitude').then(Latitude =>
          ...
          this.storage.get('Longitude').then(Longitude => 
          {
            ...
            {
              this.radiusservice.calcradius(Latitude, Longitude,umkreis, 'Community').then(pos => 
              {
                this.storage.get('Community_lat_from').then(lat_from => 
                {
                  this.storage.get('Community_lat_to').then(lat_to => 
                  {
                    this.storage.get('Community_lng_from').then(lng_from => 
                    {
                      this.storage.get('Community_lng_to').then(lng_to => 
                      {
                        console.log('Community lat_from')
                    ...
                    {
                      this.storage.set('Community_lng_to', 8.386743580769513);
                    })
                  }).catch(umkreis => 
                  {
                    this.storage.set('Community_lng_from', 8.502266419230486);
                  })
                }).catch(umkreis => 
                {
                  this.storage.set('Community_lat_to', 47.115874235794024);
                })
              }).catch(umkreis => 
              {
                this.storage.set('Community_lat_from', 47.20570576420597);
              })
            }).catch(umkreis => 
            {
    ...
    this.standardbild = this.settingservice.dog_icon;
    this.userdata = await this.ngFireAuth.currentUser;
  ...
  {
    /*
    console.log('localStorage.getItem(player_id) ' + localStorage.getItem('player_id'));
    console.log('localStorage.getItem(user) ' + localStorage.getItem('user'));
    */

    //this.onesignalservice.sendmessage('c09ac3b9-5fa8-41cf-a668-1589e11b7fc8', 'blablabla', '');
    ...
    */
    ...
    var questionsRef = this.ngFirestore.collection<communitylisting>('community', ref => ref.orderBy('timestamp', 'desc'));

    this.subscriptions = this.ngFirestore.collection<communitylisting>('community', ref => ref.orderBy('timestamp', 'desc')).snapshotChanges().subscribe();
    ...
    var questionsCol = questionsRef.snapshotChanges().pipe(
      map(actions => actions.map(a => {
         ...
         return {
          id: a.payload.doc.id,
          ...a.payload.doc.data() as CommunityLists
        };
      }))
    );
      ...
      doc.forEach(t =>
        {
          ...
          this.userdata = this.userservice.getUserBild(t.user);
          this.usersubscribe = this.userdata.subscribe(userdates => 
          {
            this.userbild = userdates[0].bild1;
  ...
  {
    this.router.navigate(['/categoriescreate']);
  }
